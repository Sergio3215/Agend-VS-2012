...
namespace frmTest
{
    public partial class frmEnglish : Form
    {
        ...
        private void Añadir()
        {
            ...
            {
                if (MessageBox.Show("The field going to be clean, Do you want continue?", "Alert", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                {
        ...
        private void Save() 
        {
            if (txtVal.TextLength > 0)
            {
                ...
                {
                    if (MessageBox.Show("You going to overwrite the selected, Do you want continue?", "Messenge",MessageBoxButtons.YesNo,MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        lbCaja.Items[lbCaja.SelectedIndex] = txtVal.Text;
                        MessageBox.Show("Correct save");
                    }
                }
            }
            Creation_Save();
        ...
        private void Delete()
        {
            if (lbCaja.SelectedIndex >= 0 && lbCaja.SelectedIndex < lbCaja.Items.Count)
            {
                if (MessageBox.Show("Do you want delete the　selected?", "Messenge", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
        ...
        {
            frmFinanciasGastosEnglish fg = new frmFinanciasGastosEnglish();
            fg.Visible = true;
            this.Visible = false;
        }
